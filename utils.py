...
# def save_image(original_images, watermarked_images, epoch, folder, resize_to=None):
#     images = original_images[:original_images.shape[0], :, :, :].cpu()
#     watermarked_images = watermarked_images[:watermarked_images.shape[0], :, :, :].cpu()
#
#     # scale values to range [0, 1] from original range of [-1, 1]
...
#
#     filename = os.path.join(folder, 'epoch-{}.png'.format(epoch))
#     torchvision.utils.save_image(watermarked_images, filename, original_images.shape[0], normalize=False)
...
def get_data_loaders(hidden_config: HiDDenConfiguration, train_options: TrainingOptions):
    ...
    train_images = datasets.ImageFolder('/new/zbb/HiDDeN-master/coco_data/', data_transforms['train'])
    train_loader = torch.utils.data.DataLoader(train_images, batch_size=train_options.batch_size, shuffle=True,
                                               num_workers=4)

    validation_images = datasets.ImageFolder('/new/zbb/HiDDeN-master/coco_data/', data_transforms['test'])
...
def zbbSave(encoder_images, epoch, path):
    for i in range(encoder_images.shape[0]):
        encoder_image = encoder_images[i].detach()
        print(encoder_image.shape)
        encoder_image = encoder_image.permute(1,2,0).cpu().numpy()
        img_path = os.path.join(path, "epoch-" + str(epoch)+ "-" + str(i) + ".png")
        cv2.imwrite(img_path, encoder_image)
