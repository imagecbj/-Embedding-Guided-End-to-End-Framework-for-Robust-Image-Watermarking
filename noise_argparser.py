...
from noise_layers.gaussnoise import Gaussnoise
...
def parse_crop(crop_command):
    matches = re.match(r'crop\((\d+\.*\d*,\d+\.*\d*)\)', crop_command)
    ratios = matches.groups()[0].split(',')
    ratio_min = float(ratios[0])
    ratio_max = float(ratios[1])
    return Crop((ratio_min, ratio_max))

def parse_gaussnoise(gaussnoise_command):
    matches = re.match(r'gaussnoise\((\d+\.*\d*,\d+\.*\d*)\)', gaussnoise_command)
    ratios = matches.groups()[0].split(',')
    ratio_min = float(ratios[0])
    ratio_max = float(ratios[1])
    return Gaussnoise((ratio_min, ratio_max))

def parse_gaussian(gaussian_command):
    matches = re.match(r'gaussian\((\d+\.*\d*,\d+\.*\d*)\)', gaussian_command)
    ratios = matches.groups()[0].split(',')
    ratio_min = float(ratios[0])
    ratio_max = float(ratios[1])
    return Gaussian(ratio_min)
...
class NoiseArgParser(argparse.Action):
    ...
    def __call__(self, parser, namespace, values,
        ...
        for command in split_commands:
            ...
            if command[:len('gaussian')] == 'gaussian':
                layers.append(parse_gaussian(command))
            elif command[:len('gaussnoise')] == 'gaussnoise':
                layers.append(parse_gaussnoise(command))
